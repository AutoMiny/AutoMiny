    ...
    if initial_line is None or scan_msg.header.stamp < initial_line.stamp:
        ...
        if (theta >0.06) and (theta<1.2) and (speed>0):
            ...
            print('current wall dist: %.3f, dist diff: %.3f, angle diff: %.1f, current angle: %.1f, start angle: %.1f, turn radius: %.3f' %
              (wall_dist, abs(dist_diff),np.rad2deg(theta), np.rad2deg(wall_angle), np.rad2deg(initial_line.wall_angle), turn_radius))
        else:
...
def main(args):
	...
	if len(args) > 1:
		try:
			...
			rospy.Subscriber("/scan", LaserScan, scan_callback, queue_size=1)  # small queue for only reading recent data
