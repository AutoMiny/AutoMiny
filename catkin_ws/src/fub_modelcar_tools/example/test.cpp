...
//    saveXML(SteeringPairs,fileName.c_str());
    ...
    for (int i=0;i<newsteeringPairs.size();i++)
        ROS_INFO_STREAM(i <<"-input command "<< newsteeringPairs.at(i).command<<", output steering "<<newsteeringPairs.at(i).steering<<" rad");
