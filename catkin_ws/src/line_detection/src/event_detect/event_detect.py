...
from clust_box import cluster_box
from event_detector import get_events
...
def callback(data):
    global frame
    global cloud_data_window
    cloud_arr = pointcloud2_to_array(data, split_rgb=True)
    cloud_cluster = cluster_box(cloud_arr)
    if len(cloud_data_window) > 50:
        cloud_data_window = cloud_data_window[1:]
    cloud_data_window.append(cloud_cluster)
    events = get_events(cloud_data_window)
    if len(events) > 0:
        for (cluster, event) in events:
            rospy.loginfo((cluster, event))
            
    #cloud_data[frame] = cloud_arr    
    #frame += 1
    #if frame < 150:
        #f = open('/tmp/cluster.p','w')
        #pickle.dump(cloud_data, f)
        #f.close()
    #else:    
        #rospy.loginfo(frame)
        #rospy.loginfo(rospy.get_name() + ": Stop Pickling for %d" % frame)

def listener():
    rospy.init_node('clust_listener', anonymous=True)
    rospy.Subscriber("obj_clusters", PointCloud2, callback)
    rospy.spin()
